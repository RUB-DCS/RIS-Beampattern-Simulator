...
%     reshapedSet = (reshape(configSet_tracking, [param.N_x*param.N_surfaces_X,param.N_y*param.N_surfaces_Y, 36]),[2,1,3]);
   ...
   configSet = reshapedSet;
    ...
    function [RxSplitPos] = generateRxSplitPositions(param)  
        ...
        RxSplitPos(1,1) = sind(param.Rx_angle_UWB_angle+(param.UWB_angle_uncertainty/2))*param.Rx_dist_UWB;
        RxSplitPos(2,1) = -cosd(param.Rx_angle_UWB_angle+(param.UWB_angle_uncertainty/2))*param.Rx_dist_UWB; 

        RxSplitPos(1,2) = sind(param.Rx_angle_UWB_angle-(param.UWB_angle_uncertainty/2))*param.Rx_dist_UWB;
        RxSplitPos(2,2) = -cosd(param.Rx_angle_UWB_angle-(param.UWB_angle_uncertainty/2))*param.Rx_dist_UWB; 
        ...
        RxSplitPos(3,:) = zeros(1,3)+param.relative_Rx_height;
